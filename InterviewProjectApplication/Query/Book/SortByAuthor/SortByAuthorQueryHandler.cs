...
{
    public class SortByAuthorQueryHandler : IRequestHandler<SortByAuthorQuery, BaseResponse<List<BookDto>>>
    {
        ...
        public SortByAuthorQueryHandler(IUnitOfWork _uow, IMapper _map)
        {
            this._uow = _uow;
            this._map = _map;
        }
        public async Task<BaseResponse<List<BookDto>>> Handle(SortByAuthorQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var sorted = _uow.GetReposiotory<InterviewProjectDomain.Model.Book>().GetListAsync().Result.OrderBy(x => x.AuthorFirstName)
