...
{
    public class SortByAuthorProcedureQueryHandler : IRequestHandler<SortByAuthorProcedureQuery, BaseResponse<List<BookDto>>>
    {
        ...
        public SortByAuthorProcedureQueryHandler(IUnitOfWork _uow, IMapper _map)
        {
            this._uow = _uow;
            this._map = _map;
        }
        public async Task<BaseResponse<List<BookDto>>> Handle(SortByAuthorProcedureQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                string query = "Exec SortByAuthor";
